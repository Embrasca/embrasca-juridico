...
module.exports = async (req, res) => {
  ...
  const r = await supabaseFetch('/auth/v1/token?grant_type=password', { method: 'POST', body: { email, password } });
  if (!r.ok || !r.data?.access_token || !r.data?.user) return json(res, 401, { error: 'E-mail ou senha inválidos.' });
