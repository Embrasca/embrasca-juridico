(() => {
  ...
  hide('#nav [data-s="dashboard"], #nav [data-s="documentos"], #nav [data-s="modelos"], #nav [data-s="config"]');
  hide('#novo [data-go="documentos"]');

  const navNew = document.querySelector('#nav [data-s="novo"]');
  if (navNew) navNew.textContent = 'Gerar documento';
  const navReview = document.querySelector('#nav [data-s="revisoes"]');
  if (navReview) navReview.textContent = 'Revisao juridica';
  ...
  if (sendButton) sendButton.textContent = 'Encaminhar para revisao juridica';
  ...
  createDoc = function () {
    ...
    if (summary) summary.insertAdjacentHTML('beforeend', '<br><br><b>Proxima etapa:</b> encaminhe o DOCX para revisao juridica antes do envio ou assinatura.');
  };
  ...
  U = { id: 'task2-internal', name: 'Equipe Embrasca', email: '', role: 'juridico', active: true };
