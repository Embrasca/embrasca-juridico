...
const TIMES_FONT = '<w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman" w:eastAsia="Times New Roman" w:cs="Times New Roman"/>';
const STANDARD_SPACING = '<w:spacing w:before="0" w:after="0" w:line="360" w:lineRule="auto"/>';

function standardizeWordFormatting(xml){
  let s=String(xml??'');
  s=s.replace(/<w:rFonts\b[^>]*\/>/g,TIMES_FONT);
  s=s.replace(/<w:rPr\b([^>]*)>([\s\S]*?)<\/w:rPr>/g,(whole,attrs,inner)=>{
    const content=/<w:rFonts\b/.test(inner)?inner:`${TIMES_FONT}${inner}`;
    return `<w:rPr${attrs}>${content}</w:rPr>`;
  });
  s=s.replace(/<w:r\b([^>]*)>([\s\S]*?)<\/w:r>/g,(whole,attrs,inner)=>{
    if(/<w:rPr\b/.test(inner)) return `<w:r${attrs}>${inner}</w:r>`;
    return `<w:r${attrs}><w:rPr>${TIMES_FONT}</w:rPr>${inner}</w:r>`;
  });
  s=s.replace(/<w:pPr\b([^>]*)>([\s\S]*?)<\/w:pPr>/g,(whole,attrs,inner)=>{
    const cleaned=inner.replace(/<w:spacing\b[^>]*\/>/g,'');
    return `<w:pPr${attrs}>${STANDARD_SPACING}${cleaned}</w:pPr>`;
  });
  s=s.replace(/<w:p\b([^>]*)>([\s\S]*?)<\/w:p>/g,(whole,attrs,inner)=>{
    if(/<w:pPr\b/.test(inner)) return `<w:p${attrs}>${inner}</w:p>`;
    return `<w:p${attrs}><w:pPr>${STANDARD_SPACING}</w:pPr>${inner}</w:p>`;
  });
  return s;
}
...
function debrandDocumentXml(xml,{preserveOriginalFooter=false}={}){
  ...
  s=s.replace(/<w:headerReference\b(?=[^>]*r:id="rIdEmbrasca[^\"]*")[^>]*\/>/g,'');
  if(!preserveOriginalFooter) s=s.replace(/<w:footerReference\b(?=[^>]*r:id="rIdEmbrasca[^\"]*")[^>]*\/>/g,'');
  s=s.replace(/<w:titlePg\s*\/>/g,'')
...
function stripEmbrascaBranding(entries,{preserveOriginalFooter=false}={}){
  ...
  for(const entry of entries){
    if(entry.name==='word/media/embrasca-logo.png' && !preserveOriginalFooter) continue;
    if(preserveOriginalFooter && /^word\/footer\d+\.xml$/i.test(entry.name)){
      out.push({...entry,data:Buffer.from(entry.data)});
      continue;
    }
    ...
    if(e.name==='word/document.xml') e.data=Buffer.from(debrandDocumentXml(e.data.toString('utf8'),{preserveOriginalFooter}),'utf8');
      ...
      s=s.replace(/<Relationship\b(?=[^>]*Id="rIdEmbrasca[^\"]*")[^>]*\/>/g,(relationship)=>{
        if(preserveOriginalFooter && /Type="[^"]*\/footer"/i.test(relationship)) return relationship;
        return '';
      });
...
function generateDocx(template,replacements,options={}){
  const preserveOriginalFooter=options.templateCode==='MINUTA_PRESTACAO_SERVICOS';
  let entries=readZip(template);
  entries=stripEmbrascaBranding(entries,{preserveOriginalFooter});
  let replaced=0;
  for(const e of entries){
    if(e.isDir||!e.name.toLowerCase().endsWith('.xml'))continue;
    if(preserveOriginalFooter && /^word\/footer\d+\.xml$/i.test(e.name)) continue;
    ...
    if(/^word\/.*\.xml$/i.test(e.name)) s=standardizeWordFormatting(s);
    e.data=Buffer.from(s,'utf8');
  }
...
module.exports={readZip,buildZip,generateDocx,escapeXml,crc32,repairWordXml,stripEmbrascaBranding,debrandDocumentXml,debrandStylesXml,standardizeWordFormatting};
